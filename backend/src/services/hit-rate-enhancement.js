...
function buildRacingStyleProfile(row) {
  const styleProfile = row?.style_profile || {};
  const sashi = toNum(styleProfile?.sashi, 0) / 100;
  const makuri = toNum(styleProfile?.makuri, 0) / 100;
  const makuriSashi = toNum(styleProfile?.makuri_sashi, 0) / 100;
  const ranking = [
    { key: "sashi", value: sashi },
    { key: "makuri", value: makuri },
    { key: "makuri_sashi", value: makuriSashi }
  ].sort((a, b) => b.value - a.value);
  const top = ranking[0] || { key: "balanced", value: 0 };
  const second = ranking[1] || { key: "balanced", value: 0 };
  const style =
    top.value >= 0.48 || (top.value >= 0.4 && top.value - second.value >= 0.08)
      ? top.key
      : "balanced";
  return {
    style,
    style_rates: {
      sashi: round(sashi, 4),
      makuri: round(makuri, 4),
      makuri_sashi: round(makuriSashi, 4)
    }
  };
}

function buildCapabilityProfile(row, actualLaneMap) {
  const style = String(row?.style || "balanced");
  const actualLane = actualLaneOf(row);
  const startEdge = toNum(row?.start_edge, 0);
  const motor = clamp(0, 1, toNum(row?.motor_true, 0) / 100);
  const exhibitionStNorm = Number.isFinite(row?.exhibition_st)
    ? clamp(0, 1, (0.19 - toNum(row?.exhibition_st, 0.19)) / 0.08)
    : 0.45;
  const lapNorm = row?.prediction_data_usage?.lapTime?.used
    ? clamp(0, 1, Math.max(0, 6.88 - toNum(row?.motor_form?.lapTime, 6.88)) / 0.16)
    : 0.4;
  const exTimeNorm = Number.isFinite(row?.ex_time_relative_gap)
    ? clamp(0, 1, Math.max(0, 0.1 - toNum(row?.ex_time_relative_gap, 0.1)) / 0.1)
    : 0.4;
  const turningNorm = Number.isFinite(row?.turning_ability)
    ? clamp(0, 1, (toNum(row?.turning_ability, 4.8) - 4.6) / 1.4)
    : clamp(0, 1, Math.max(0, toNum(row?.finish_role_bonuses?.turningAbilityDelta, 0)) * 4.2);
  const motorRaw = row?.prediction_data_usage?.motor2ren?.used ? clamp(0, 1, toNum(row?.motor_raw?.motor2ren, 0) / 100) : motor;
  const styleMakuriBonus = style === "makuri" ? 0.12 : style === "makuri_sashi" ? 0.08 : 0;
  const styleSashiBonus = style === "sashi" ? 0.12 : style === "makuri_sashi" ? 0.08 : 0;
  const attackCapability = clamp(
    0,
    1,
    clamp(0, 1, (startEdge + 0.03) / 0.13) * 0.42 +
      exhibitionStNorm * 0.16 +
      motorRaw * 0.2 +
      motor * 0.12 +
      styleMakuriBonus +
      toNum(row?.monster_motor_attack_support, 0) * 0.42
  );
  const turnEfficiency = clamp(
    ...
    turningNorm * 0.42 +
      lapNorm * 0.26 +
      Math.max(0, toNum(row?.finish_role_bonuses?.turningAbilityDelta, 0)) * 0.12 +
      styleSashiBonus
  );
  const recoveryPower = clamp(
    0,
    1,
    lapNorm * 0.32 +
      exTimeNorm * 0.28 +
      motorRaw * 0.2 +
      motor * 0.12 +
      Math.max(0, toNum(row?.finish_role_bonuses?.straightLineDelta, 0)) * 0.12
  );
  const neighborEdges = [actualLane - 1, actualLane + 1]
    .map((lane) => actualLaneMap.get(lane)?.start_edge)
    .filter((value) => Number.isFinite(value));
  const neighborAvgStart = average(neighborEdges);
  const slowStVsNeighbors = Number.isFinite(neighborAvgStart) ? startEdge <= neighborAvgStart - 0.015 : startEdge <= 0;
  const fastSt = startEdge >= 0.04;
  const secondAdjustment =
    slowStVsNeighbors && (style === "sashi" || style === "makuri_sashi") && turnEfficiency >= 0.58
      ? clamp(0, 0.12, 0.04 + turnEfficiency * 0.09 + (style === "sashi" ? 0.012 : 0))
      : 0;
  const thirdAdjustment =
    slowStVsNeighbors && recoveryPower >= 0.56
      ? clamp(0, 0.1, 0.026 + recoveryPower * 0.08 + (actualLane >= 4 ? 0.01 : 0))
      : 0;
  const failureRisk =
    fastSt && style === "makuri" && motorRaw < 0.5
      ? clamp(0, 0.12, 0.03 + (0.5 - motorRaw) * 0.12 + attackCapability * 0.04)
      : 0;
  ...
  return {
    ...
    st_vs_result_adjustment: {
      second: round(secondAdjustment, 4),
      third: round(thirdAdjustment, 4),
      failure_risk: round(failureRisk, 4),
...
function buildFinishRoleScores(laneContexts, laneMap, actualLaneMap, baseRoleProbabilities, scenarioRows, escapeScore, laneFinishPriors) {
  ...
  for (const row of laneContexts) {
    ...
    const attackCapability = Math.max(0, toNum(row?.attack_capability, 0));
    const turnEfficiency = Math.max(0, toNum(row?.turn_efficiency, 0));
    const recoveryPower = Math.max(0, toNum(row?.recovery_power, 0));
    const stVsResultAdjustment = row?.st_vs_result_adjustment || { second: 0, third: 0, failure_risk: 0 };
    ...
    const boat1BaseFirstBonus = boatNumberOf(row) === 1 && actualLane === 1
      ? clamp(
          ...
          0.33,
            ...
            toNum(row?.venue_inside_finish_bias, 0) * 0.84
        )
      : 0;
    const boat1SurvivalBonus = boatNumberOf(row) === 1 && actualLane === 1
      ? clamp(0, 0.23, 0.07 + stableFinishBonus * 0.34 + laneFit2 * 0.04 + laneFit3 * 0.03 + toNum(row?.venue_inside_second_third_bias, 0) * 0.86)
      : 0;
    ...
    const insideSecondThirdRecoveryAfter = {
      second:
        actualLane === 2 && primaryHeadLane === 1
          ? clamp(0, 0.145, laneFit2 * 0.068 + Math.max(0, turning) * 0.048 + Math.max(0, 0.54 - escapeScore) * 0.022 + toNum(row?.venue_inside_second_third_bias, 0) * 0.92)
          : 0,
      third:
        actualLane === 3 && primaryHeadLane === 1
          ? clamp(0, 0.125, laneFit3 * 0.052 + Math.max(0, turning) * 0.04 + Math.max(0, straight) * 0.016 + toNum(row?.venue_inside_second_third_bias, 0) * 0.82)
          : 0
    };
    ...
    const secondPlaceScore =
      secondPlaceScoreBeforeTuning +
      toNum(stVsResultAdjustment?.second, 0) +
      actualFourSecondCarryover +
      insideSecondThirdRecoveryAfter.second +
      boat2SecondRecoveryAfter +
      boat1SurvivalBonus * 0.4;
    const thirdPlaceScore =
      thirdPlaceScoreBeforeTuning +
      toNum(stVsResultAdjustment?.third, 0) +
      actualFourThirdCarryover +
      insideSecondThirdRecoveryAfter.third +
    ...
    row.finish_role_scores = {
      ...
      st_vs_result_adjustment: {
        ...
        failure_risk: round(toNum(stVsResultAdjustment?.failure_risk, 0), 4)
      },
...
function roleScenarioWeightsForLane(lane, row, scenario, events, escapeScore, actualLaneMap) {
  ...
  const outsideHeadSuppression = toNum(row?.outside_head_suppression_amount, 0);
  const boat1BaseFirstBonus = boatNumberOf(row) === 1 && actualLane === 1
...
function getVenueInsideTier(value) {
  const normalized = normalizeVenueName(value);
  if ([
    "tokuyama",
    "omura",
  ...
  return isStrongInsideVenue(value) ? "medium" : "default";
}
...
function buildVenueBiasContext({ race, raceFlow, laneContexts }) {
  ...
  const venueInsideTier = getVenueInsideTier(venueName);
  const strongTierBoost = venueInsideTier === "strong" ? 1 : 0;
  const mediumTierBoost = venueInsideTier === "medium" ? 1 : 0;
  ...
  const venueEscapeBias = clamp(
    ...
    insideStrengthDelta * 0.012 +
      ...
      windSpeed * 0.003 +
      (strongTierBoost ? 0.036 : mediumTierBoost ? 0.026 : strongInsideVenue ? 0.02 : 0)
  );
  const venueInsideFinishBias = clamp(
    ...
    Math.max(0, insideStrengthDelta) * 0.008 +
      ...
      (strongTierBoost ? 0.04 : mediumTierBoost ? 0.026 : strongInsideVenue ? 0.018 : 0.01)
  );
  const venueInsideSecondThirdBias = clamp(
    ...
    Math.max(0, insideStrengthDelta) * 0.006 +
      ...
      (strongTierBoost ? 0.03 : mediumTierBoost ? 0.02 : strongInsideVenue ? 0.014 : 0.008)
  );
  const outsideHeadSuppression = clamp(
    0,
    0.08,
    (strongTierBoost ? 0.045 : mediumTierBoost ? 0.024 : strongInsideVenue ? 0.015 : 0) +
      Math.max(0, insideStrengthDelta) * 0.002
  );
  const venueOuterAttackBias = clamp(0, 0.08, Math.max(0, -insideStrengthDelta) * 0.01 + Math.max(0, windSpeed - 4) * 0.006 + (String(raceFlow?.formation_pattern || "").includes("outside") ? 0.018 : 0) - outsideHeadSuppression * 0.72);
  ...
  return {
    ...
    venue_inside_tier: venueInsideTier,
    ...
    outside_head_suppression_amount: round(outsideHeadSuppression, 4),
...
export function buildHitRateEnhancementContext({
  ...
  laneContexts.forEach((row) => {
    ...
    row.venue_inside_tier = venueBias.venue_inside_tier || "default";
    ...
    row.outside_head_suppression_amount = venueBias.outside_head_suppression_amount;
    row.finish_role_bonuses = buildRoleSpecificFinishBonuses(row, actualLaneMap);
    Object.assign(row, buildRacingStyleProfile(row));
  ...
  const boat3HeadPromotionBefore = clamp(
    ...
    (toNum(actualLane3Row?.style_profile?.makuri, 0) / 100) * 0.48 +
      ...
      Math.max(0, 0.45 - escapeScoreBeforeActualEntryAdjust) * 0.28 -
      toNum(venueBias?.outside_head_suppression_amount, 0) * 0.62
  );
  ...
  const actualFourCadoBasePriority = clamp(
    ...
    (toNum(actualLane4Row?.style_profile?.makuri, 0) / 100) * 0.34 +
      ...
      (startPatternContext.outer_attack_window ? 0.05 : 0) -
      toNum(venueBias?.outside_head_suppression_amount, 0) * 0.7 -
      toNum(actualLane4Row?.late_risk, 0) * 0.16 -
      toNum(actualLane4Row?.hidden_F_flag, 0) * 0.08
  );
  const actualFourBoostGate = entryStructureChanged || boatNumberOf(actualLane4Row) !== 4 || startPatternContext.outer_attack_window ? clamp(0.72, 1, 1 - toNum(venueBias?.outside_head_suppression_amount, 0) * 2.4) : 0.08;
  const actualFourCadoBoost = clamp(
    ...
    ((toNum(actualLane4Row?.style_profile?.makuri, 0) / 100) * 0.06 +
      ...
      toNum(venueBias?.outside_head_suppression_amount, 0) * 0.52 -
  ...
  laneFinishPriors.second[2] = round(
    clamp(
      1.08,
      1.22,
      toNum(laneFinishPriors.second[2], 1.06) + toNum(venueBias?.venue_inside_second_third_bias, 0) * 0.62
    ),
    4
  );
  laneFinishPriors.third[2] = round(
    clamp(
      1.02,
      1.15,
      toNum(laneFinishPriors.third[2], 1.02) + toNum(venueBias?.venue_inside_second_third_bias, 0) * 0.34
    ),
    4
  );
  laneFinishPriors.third[3] = round(
    clamp(
      1.01,
      1.13,
      toNum(laneFinishPriors.third[3], 1.01) + toNum(venueBias?.venue_inside_second_third_bias, 0) * 0.26
    ),
    4
  );
  laneFinishPriors.third[6] = round(
    clamp(
      0.88,
      0.98,
      toNum(laneFinishPriors.third[6], 0.88) + toNum(venueBias?.venue_inside_second_third_bias, 0) * 0.12
    ),
    4
  );
  ...
  return {
    ...
    stage2_dynamic: {
      start_development_states: Object.fromEntries(laneContexts.map((row) => [String(row.lane), {
        ...
        outside_head_suppression_amount: row.outside_head_suppression_amount,
